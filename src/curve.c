...
static GnoclOption curveOptions[] =
{
	...
	/* handle events */
	{ "-onCurvedChanged", GNOCL_OBJ, "curve-type-changed" , gnoclOptOnCurveChanged},
	{ NULL },
};
...
static int configure ( Tcl_Interp *interp, GtkCurve *widget, GnoclOption options[] )
{
	...
	if ( options[CurveTypeIdx].status == GNOCL_STATUS_CHANGED )
	{
		...
		if ( !strcmp ( options[CurveTypeIdx].val.str, "linear" ) )
		{
			printf ( "1 gnoclOptCurvetype = linear\n" );
			gtk_curve_set_curve_type ( GTK_CURVE ( widget ), GTK_CURVE_TYPE_LINEAR );

		}

		/* spline interpolation */
		if ( !strcmp ( options[CurveTypeIdx].val.str, "spline" ) )
		{

			printf ( "2 gnoclOptCurvetype = spline\n" );
			gtk_curve_set_curve_type ( GTK_CURVE ( widget ), GTK_CURVE_TYPE_SPLINE );

		}

		/* free form curve */
		if ( !strcmp ( options[CurveTypeIdx].val.str, "free" ) )
		{
			printf ( "3 gnoclOptCurvetype = free\n" );
