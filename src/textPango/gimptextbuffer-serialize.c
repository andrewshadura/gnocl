...
\brief
**/
static void find_list_delta ( GSList *old_list, GSList *new_list, GList **added, GList **removed )
{
	GSList *tmp;
...
\brief
**/
static gboolean open_tag ( GimpTextBuffer *buffer, GString *string, GtkTextTag *tag )
{
	const gchar *name;
	const gchar *attribute;
	gchar *attribute_value;

	name = gimp_text_buffer_tag_to_name ( buffer, tag, &attribute, &attribute_value );

	if ( name )
	{
		if ( attribute && attribute_value )
		{
			g_string_append_printf ( string, "<%s %s=\"%s\">", name, attribute, attribute_value );
...
\brief
**/
static gboolean close_tag ( GimpTextBuffer *buffer, GString *string, GtkTextTag *tag )
{
...
\brief
**/
guint8 *gimp_text_buffer_serialize ( GtkTextBuffer *register_buffer, GtkTextBuffer *content_buffer, const GtkTextIter *start, const GtkTextIter *end, gsize *length, gpointer user_data )
{
	GString *string;
			...
			*/
			if ( g_slist_find ( active_tags, tag ) )
			{
				...
				*/
				...
				{
					close_tag ( GIMP_TEXT_BUFFER ( register_buffer ), string, active_tags->data );
...
\brief
**/
static void set_error ( GError **err, GMarkupParseContext *context, int error_domain, int error_code, const char *format, ... )
{
	gint line, ch;
	va_list args;
	gchar *str;
	...
	g_set_error ( err, error_domain, error_code, ( "Line %d character %d: %s" ), line, ch, str );
...
static gboolean check_no_attributes ( GMarkupParseContext *context, const char *element_name, const char **attribute_names, const char **attribute_values, GError **error )
{
	...
	{
		set_error ( error, context,	G_MARKUP_ERROR,	G_MARKUP_ERROR_PARSE, _ ( "Attribute \"%s\" is invalid on <%s> element in this context" ), attribute_names[0], element_name );
		return FALSE;
	}
...
**/
static void
parse_tag_element ( GMarkupParseContext *context, const gchar *element_name, const gchar **attribute_names, const gchar **attribute_values, ParseInfo *info, GError **error )
{
	GtkTextTag *tag;
	...
	tag = gimp_text_buffer_name_to_tag ( GIMP_TEXT_BUFFER ( info->register_buffer ), element_name, attribute_name, attribute_value );
...
static void start_element_handler ( GMarkupParseContext *context, const gchar *element_name, const gchar **attribute_names, const gchar **attribute_values,	gpointer user_data,	GError **error )
{
	...
	switch ( peek_state ( info ) )
	{
		case STATE_START:
			{
				...
				{
					if ( ! check_no_attributes ( context, element_name, attribute_names, attribute_values, error ) )
					{
						return;
					}
				...
				{
					set_error ( error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE, _ ( "Outermost element in text must be <markup> not <%s>" ), element_name );
				}
		...
		case STATE_UNKNOWN:
			{
				parse_tag_element ( context, element_name, attribute_names, attribute_values, info, error );
			}
			break;
...
\brief
**/
static gboolean all_whitespace ( const char *text, gint text_len )
{
	const char *p = text;
...
static void text_handler ( GMarkupParseContext *context, const gchar *text, gsize text_len, gpointer user_data, GError **error )
{
	...
	if ( all_whitespace ( text, text_len ) && peek_state ( info ) != STATE_MARKUP && peek_state ( info ) != STATE_TAG && peek_state ( info ) != STATE_UNKNOWN )
	{
		return;
	}
...
\brief
**/
static void parse_info_init ( ParseInfo *info, GtkTextBuffer *register_buffer, GtkTextBuffer *content_buffer )
{
...
\brief
**/
static void text_span_free ( TextSpan *span )
{
	g_free ( span->text );
...
\brief
**/
static void insert_text ( ParseInfo *info, GtkTextIter *iter )
{
	...
	mark = gtk_text_buffer_create_mark ( info->content_buffer, "deserialize-insert-point", &start_iter, TRUE );
	...
	for ( tmp = info->spans; tmp; tmp = tmp->next )
	{
		...
		for ( tags = span->tags; tags; tags = tags->next )
		{
			...
			gtk_text_buffer_apply_tag ( info->content_buffer, tag, &start_iter, iter );
		}
...
\brief
**/
gboolean gimp_text_buffer_deserialize ( GtkTextBuffer *register_buffer, GtkTextBuffer *content_buffer, GtkTextIter *iter, const guint8 *text, gsize length, gboolean create_tags, gpointer user_data, GError **error )
{
	...
	if ( ! g_markup_parse_context_parse ( context, ( const gchar * ) text, length, error ) )
	{
		goto out;
	}
...
\brief
**/
void gimp_text_buffer_pre_serialize ( GimpTextBuffer *buffer, GtkTextBuffer *content )
{
	GtkTextIter iter;
	...
	do
	{
		...
		for ( list = tags; list; list = g_slist_next ( list ) )
		{
			...
			if ( g_list_find ( buffer->kerning_tags, tag ) )
			{
				...
				gtk_text_buffer_insert_with_tags ( content, &iter, WORD_JOINER, -1, tag, NULL );
...
\brief
**/
void gimp_text_buffer_post_deserialize ( GimpTextBuffer *buffer, GtkTextBuffer *content )
{
	GtkTextIter iter;
	...
	do
	{
		...
		for ( list = tags; list; list = g_slist_next ( list ) ) /* 1	Loop condition is true. Entering loop body */
		{
			GtkTextTag *tag = list->data;

			if ( g_list_find ( buffer->kerning_tags, tag ) ) /* 2 Access to field 'kerning_tags' results in a dereference of a null pointer (loaded from variable 'buffer') */
			{
				GtkTextIter end;
