...
static GObject * gimp_text_buffer_constructor ( GType type,	guint n_params,	GObjectConstructParam *params );
static void gimp_text_buffer_dispose ( GObject *object );
static void gimp_text_buffer_finalize ( GObject *object );
static void gimp_text_buffer_mark_set ( GtkTextBuffer *buffer, const GtkTextIter *location, GtkTextMark *mark );
...
\brief
**/
static void gimp_text_buffer_class_init ( GimpTextBufferClass *klass )
{
...
\brief
**/
static void gimp_text_buffer_init ( GimpTextBuffer *buffer )
{
	buffer->markup_atom = gtk_text_buffer_register_serialize_format ( GTK_TEXT_BUFFER ( buffer ),	"application/x-gimp-pango-markup", gimp_text_buffer_serialize,	NULL, NULL );

	gtk_text_buffer_register_deserialize_format ( GTK_TEXT_BUFFER ( buffer ), "application/x-gimp-pango-markup", gimp_text_buffer_deserialize, NULL, NULL );
}
...
static GObject * gimp_text_buffer_constructor ( GType type, guint n_params, GObjectConstructParam *params )
{
	...
	buffer->bold_tag = gtk_text_buffer_create_tag ( GTK_TEXT_BUFFER ( buffer ), "bold", "weight", PANGO_WEIGHT_BOLD, NULL );
	buffer->italic_tag = gtk_text_buffer_create_tag ( GTK_TEXT_BUFFER ( buffer ), "italic", "style", PANGO_STYLE_ITALIC, NULL );
	buffer->underline_tag = gtk_text_buffer_create_tag ( GTK_TEXT_BUFFER ( buffer ), "underline", "underline", PANGO_UNDERLINE_SINGLE, NULL );
	buffer->strikethrough_tag = gtk_text_buffer_create_tag ( GTK_TEXT_BUFFER ( buffer ), "strikethrough", "strikethrough", TRUE, NULL );
...
\brief
**/
void gimp_text_buffer_set_text ( GimpTextBuffer *buffer, const gchar *text )
{
...
\brief
**/
void gimp_text_buffer_set_markup ( GimpTextBuffer *buffer, const gchar *markup )
{
	g_return_if_fail ( GIMP_IS_TEXT_BUFFER ( buffer ) );
...
\brief
**/
GtkTextTag *gimp_text_buffer_get_iter_size ( GimpTextBuffer *buffer, const GtkTextIter *iter, gint *size )
{
	GList *list;
...
\brief
**/
static GtkTextTag *gimp_text_buffer_get_size_tag ( GimpTextBuffer *buffer, gint size )
{
	GList *list;
	GtkTextTag *tag;
	gchar name[32];
...
\brief
**/
void gimp_text_buffer_set_size ( GimpTextBuffer *buffer, const GtkTextIter *start, const GtkTextIter *end, gint size )
{
	GList *list;
	...
	for ( list = buffer->size_tags; list; list = g_list_next ( list ) )
	{
		gtk_text_buffer_remove_tag ( GTK_TEXT_BUFFER ( buffer ), list->data, start, end );
	}
...
\brief
**/
void gimp_text_buffer_change_size ( GimpTextBuffer *buffer, const GtkTextIter *start, const GtkTextIter *end, gint count )
{
	GtkTextIter iter;
	...
	iter = *start;
	span_start = *start;
	span_tag = gimp_text_buffer_get_iter_size ( buffer, &iter, &span_size );
	...
	do
	{
		...
		iter_tag = gimp_text_buffer_get_iter_size ( buffer, &iter, &iter_size );
				...
				gtk_text_iter_compare ( &iter, end ) >= 0 )
		{
			if ( span_size != 0 )
			{
				gtk_text_buffer_remove_tag ( GTK_TEXT_BUFFER ( buffer ), span_tag, &span_start, &span_end );
			}

			if ( ( span_size + count ) > 0 )
			{
				span_tag = gimp_text_buffer_get_size_tag ( buffer, span_size + count );

				gtk_text_buffer_apply_tag ( GTK_TEXT_BUFFER ( buffer ), span_tag, &span_start, &span_end );
			}
...
\brief
**/
GtkTextTag *gimp_text_buffer_get_iter_baseline ( GimpTextBuffer *buffer, const GtkTextIter *iter, gint *baseline )
{
	GList *list;
...
\brief
**/
static GtkTextTag *gimp_text_buffer_get_baseline_tag ( GimpTextBuffer *buffer, gint baseline )
{
	GList *list;
	GtkTextTag *tag;
	gchar name[32];
	...
	tag = gtk_text_buffer_create_tag ( GTK_TEXT_BUFFER ( buffer ), name, GIMP_TEXT_PROP_NAME_BASELINE, baseline, NULL );
...
**/
void
gimp_text_buffer_set_baseline ( GimpTextBuffer *buffer, const GtkTextIter *start, const GtkTextIter *end, gint baseline )
{
	GList *list;
...
\brief
**/
void gimp_text_buffer_change_baseline ( GimpTextBuffer *buffer, const GtkTextIter *start, const GtkTextIter *end, gint count )
{
	GtkTextIter iter;
	GtkTextIter span_start;
	GtkTextIter span_end;
	...
	iter = *start;
	span_start = *start;
	span_tag = gimp_text_buffer_get_iter_baseline ( buffer, &iter, &span_baseline );
	...
	do
	{
		...
		iter_tag = gimp_text_buffer_get_iter_baseline ( buffer, &iter, &iter_baseline );

		span_end = iter;

		if ( iter_baseline != span_baseline || gtk_text_iter_compare ( &iter, end ) >= 0 )
		{
			if ( span_baseline != 0 )
			{
				gtk_text_buffer_remove_tag ( GTK_TEXT_BUFFER ( buffer ), span_tag, &span_start, &span_end );
			...
			{
				span_tag = gimp_text_buffer_get_baseline_tag ( buffer, span_baseline + count );
...
\brief
**/
GtkTextTag *gimp_text_buffer_get_iter_kerning ( GimpTextBuffer *buffer, const GtkTextIter *iter, gint *kerning )
{
	GList *list;
...
\brief
**/
static GtkTextTag *gimp_text_buffer_get_kerning_tag ( GimpTextBuffer *buffer, gint kerning )
{
	GList *list;
	GtkTextTag *tag;
	gchar name[32];
	...
	tag = gtk_text_buffer_create_tag ( GTK_TEXT_BUFFER ( buffer ), name, GIMP_TEXT_PROP_NAME_KERNING, kerning, NULL );
...
**/
void
gimp_text_buffer_set_kerning ( GimpTextBuffer *buffer, const GtkTextIter *start, const GtkTextIter *end, gint kerning )
{
	GList *list;
...
\brief
**/
void gimp_text_buffer_change_kerning ( GimpTextBuffer *buffer, const GtkTextIter *start, const GtkTextIter *end, gint count )
{
	GtkTextIter iter;
	GtkTextIter span_start;
	GtkTextIter span_end;
	...
	iter = *start;
	span_start = *start;
	span_tag = gimp_text_buffer_get_iter_kerning ( buffer, &iter, &span_kerning );
	...
	do
	{
		...
		iter_tag = gimp_text_buffer_get_iter_kerning ( buffer, &iter, &iter_kerning );

		span_end = iter;

		if ( iter_kerning != span_kerning || gtk_text_iter_compare ( &iter, end ) >= 0 )
		{
			if ( span_kerning != 0 )
			{
				gtk_text_buffer_remove_tag ( GTK_TEXT_BUFFER ( buffer ), span_tag, &span_start, &span_end );
			...
			{
				span_tag = gimp_text_buffer_get_kerning_tag ( buffer, span_kerning + count );
...
\brief
**/
GtkTextTag *gimp_text_buffer_get_iter_font ( GimpTextBuffer *buffer, const GtkTextIter *iter, gchar **font )
{
	GList *list;
...
\brief
**/
static GtkTextTag *gimp_text_buffer_get_font_tag ( GimpTextBuffer *buffer, const gchar *font )
{
	GList *list;
	GtkTextTag *tag;
	gchar name[256];
	...
	tag = gtk_text_buffer_create_tag ( GTK_TEXT_BUFFER ( buffer ), name, "font", font, NULL );
...
\brief
**/
void gimp_text_buffer_set_font ( GimpTextBuffer *buffer, const GtkTextIter *start, const GtkTextIter *end, const gchar *font )
{
	GList *list;
...
\brief
**/
GtkTextTag *gimp_text_buffer_get_iter_color ( GimpTextBuffer *buffer, const GtkTextIter *iter, GimpRGB *color )
{
	GList *list;
...
\brief
**/
static GtkTextTag *gimp_text_buffer_get_color_tag ( GimpTextBuffer *buffer, const GimpRGB *color )
{
	GList *list;
	GtkTextTag *tag;
	gchar name[256];
	...
	for ( list = buffer->color_tags; list; list = g_list_next ( list ) )
	{
		...
		if ( tag_r == r && tag_g == g && tag_b == b )
		{
			return tag;
		}
	}

	g_snprintf ( name, sizeof ( name ), "color-#%02x%02x%02x", r, g, b );

	gimp_rgb_get_gdk_color ( color, &gdk_color );

	tag = gtk_text_buffer_create_tag ( GTK_TEXT_BUFFER ( buffer ), name, "foreground-gdk", &gdk_color, "foreground-set", TRUE, NULL );
...
\brief
**/
void gimp_text_buffer_set_color ( GimpTextBuffer *buffer, const GtkTextIter *start, const GtkTextIter *end, const GimpRGB *color )
{
	GList *list;
...
\brief
**/
const gchar *gimp_text_buffer_tag_to_name ( GimpTextBuffer *buffer, GtkTextTag *tag, const gchar **attribute, gchar **value )
{
	g_return_val_if_fail ( GIMP_IS_TEXT_BUFFER ( buffer ), NULL );
...
\brief
**/
GtkTextTag *gimp_text_buffer_name_to_tag ( GimpTextBuffer *buffer, const gchar *name, const gchar *attribute, const gchar *value )
{
	g_return_val_if_fail ( GIMP_IS_TEXT_BUFFER ( buffer ), NULL );
	g_return_val_if_fail ( name != NULL, NULL );

	if ( ! strcmp ( name, "b" ) ) /* 1 Taking false branch */
	{
		return buffer->bold_tag;
	}

	else if ( ! strcmp ( name, "i" ) ) /* 2 Taking true branch */
	{
		return buffer->italic_tag; /* 3 Access to field 'italic_tag' results in a dereference of a null pointer (loaded from variable 'buffer') */
	}
	...
	else if ( ! strcmp ( name, "span" ) && attribute != NULL && value != NULL )
	{
...
\brief
**/
void gimp_text_buffer_set_insert_tags ( GimpTextBuffer *buffer, GList *insert_tags, GList *remove_tags )
{
	g_return_if_fail ( GIMP_IS_TEXT_BUFFER ( buffer ) );
...
\brief
**/
void gimp_text_buffer_insert ( GimpTextBuffer *buffer, const gchar *text )
{
	...
	if ( insert_tags_set )
	{
		...
		for ( list = remove_tags; list; list = g_list_next ( list ) )
		{
			...
			gtk_text_buffer_remove_tag ( GTK_TEXT_BUFFER ( buffer ), tag, &start, &iter );
		}
		...
		for ( list = insert_tags; list; list = g_list_next ( list ) )
		{
			...
			gtk_text_buffer_apply_tag ( GTK_TEXT_BUFFER ( buffer ), tag, &start, &iter );
		}
	}
	...
	if ( tags_off )
	{
		...
		for ( slist = tags_off; slist; slist = g_slist_next ( slist ) )
		{
			...
			if ( ! g_list_find ( remove_tags, tag ) && ! g_list_find ( buffer->kerning_tags, tag ) )
			{
				gtk_text_buffer_apply_tag ( GTK_TEXT_BUFFER ( buffer ), tag, &start, &iter );
			}
		}
...
\brief
**/
gint gimp_text_buffer_get_iter_index ( GimpTextBuffer *buffer, GtkTextIter *iter, gboolean layout_index )
{
	GtkTextIter start;
	gchar *string;
	gint index;
	...
	string = gtk_text_buffer_get_text ( GTK_TEXT_BUFFER ( buffer ),	&start, iter, TRUE );
	index = strlen ( string );
	g_free ( string );
...
\brief
**/
void gimp_text_buffer_get_iter_at_index ( GimpTextBuffer *buffer, GtkTextIter *iter, gint index, gboolean layout_index )
{
	GtkTextIter start;
	GtkTextIter end;
	gchar *string;
	...
	string = gtk_text_buffer_get_text ( GTK_TEXT_BUFFER ( buffer ),	&start, &end, TRUE );
	...
	if ( layout_index )
	{
		...
		while ( my_index < index && ! gtk_text_iter_equal ( &start, &end ) );
	}

	string[index] = '\0';

	gtk_text_buffer_get_iter_at_offset ( GTK_TEXT_BUFFER ( buffer ), iter, g_utf8_strlen ( string, -1 ) );
...
\brief
**/
gboolean gimp_text_buffer_load ( GimpTextBuffer *buffer, const gchar *filename, GError **error )
{
	FILE *file;
	gchar buf[2048];
	gint remaining = 0;
	...
	while ( ! feof ( file ) )
	{
		...
		gtk_text_buffer_insert ( GTK_TEXT_BUFFER ( buffer ), &iter, buf, leftover - buf );
		gtk_text_buffer_get_end_iter ( GTK_TEXT_BUFFER ( buffer ), &iter );
	...
	if ( remaining )
	{
		g_message ( _ ( "Invalid UTF-8 data in file '%s'." ), gimp_filename_to_utf8 ( filename ) );
	}
...
\brief
**/
gboolean gimp_text_buffer_save ( GimpTextBuffer *buffer, const gchar *filename, gboolean selection_only, GError **error )
{
	...
	if ( selection_only )
	{
		gtk_text_buffer_get_selection_bounds ( GTK_TEXT_BUFFER ( buffer ), &start_iter, &end_iter );
	}
	...
	text_contents = gtk_text_buffer_get_text ( GTK_TEXT_BUFFER ( buffer ), &start_iter, &end_iter, TRUE );
