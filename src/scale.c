...
/**
\page page_scale gnocl::scale
\htmlinclude scale.html
**/
...
static int doCommand ( ScaleParams *para, Tcl_Obj *val, int background )
{
	if ( para->onValueChanged )
	{
		...
		return gnoclPercentSubstAndEval ( para->interp, ps, para->onValueChanged, background );
	}
		...
		case CgetIdx:
			{
				...
				switch ( gnoclCget ( interp, objc, objv, G_OBJECT ( para->scale ), scaleOptions, &idx ) )
				{
