...
/**
\page page_menuItem gnocl::menuItem
\\htmlinclude menuItem.html
**/
...
static int configure ( Tcl_Interp *interp, GtkMenuItem *label, GnoclOption options[] )
{
	...
	if ( options[iconIdx].status == GNOCL_STATUS_CHANGED )
	{
		...
		if ( gnoclGetImage ( interp, options[iconIdx].val.obj, GTK_ICON_SIZE_MENU, &image ) != TCL_OK )
		{
			return TCL_ERROR;
		}

		/* if icon == "", ret == TCL_OK, but image == NULL */
		/* if an icon is set, it will be shown by default */
		if ( image )
		{
			gtk_widget_show ( GTK_WIDGET ( image ) );
			gtk_image_menu_item_set_image ( GTK_IMAGE_MENU_ITEM ( label ), image );
			gtk_image_menu_item_set_always_show_image ( label, 1 );
		}
	}

	/* option necessary from Gtk+ 2.16 onwards */
	if ( options[showImageIdx].status == GNOCL_STATUS_CHANGED )
	{
...
\brief
**/
int gnoclMenuItemCmd ( ClientData data, Tcl_Interp *interp, int objc, Tcl_Obj * const objv[] )
{
