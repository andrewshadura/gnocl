/**
\brief	This module implements the gnocl::inputDialog widget.
\todo
    The dialog window is not closing whenever the close button is cliked.
    Looking at the code, the widget is displayed using the gtk_widget_show function.
...
/**
\page page_inputDialog gnocl::inputDialog
\htmlinclude inputDialog.html
**/
...
int gnoclinputDialogCmd ( ClientData data, Tcl_Interp *interp, int objc, Tcl_Obj * const objv[] )
{
	...
	if ( gnoclGetCmdsAndOpts ( interp, cmds, inputDialogOptions, objv, objc ) == TCL_OK )
	{
		return TCL_OK;
	}
